...
MODULE_DESCRIPTION("PL330 DMA Controller driver for communication between application and FPGA.");
MODULE_VERSION("1.0");
...
//-------------VARIABLES TO DO LOCKDOWN ON L2 CACHE CONTROLLER---------//
...
static ssize_t dma_buff_padd_store(struct kobject *kobj,
...
static ssize_t dma_buff_padd_show(struct kobject *kobj,
...
static ssize_t use_acp_store(struct kobject *kobj,
  struct kobj_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t use_acp_show(struct kobject *kobj,
  struct kobj_attribute *attr, char *buf)
{
...
static ssize_t prepare_microcode_in_open_show(struct kobject *kobj,
  struct kobj_attribute *attr, char *buf)
{
   return sprintf(buf, "%d\n", prepare_microcode_in_open);
}

static ssize_t dma_transfer_size_store(struct kobject *kobj,
  struct kobj_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t dma_transfer_size_show(struct kobject *kobj,
  struct kobj_attribute *attr, char *buf)
{
   return sprintf(buf, "%d\n", dma_transfer_size);
}
...
static ssize_t lockdown_cpu_show(struct kobject *kobj,
  struct kobj_attribute *attr, char *buf)
{
   return sprintf(buf, "%d\n", lockdown_cpu);
}
  ...
  struct kobj_attribute *attr, const char *buf, size_t count)
{
  ...
  {
    //Define AXI ID for masters writing/reading in/from ACP. We configured ACP ID
    //Mapper in init of the module so all masters accessing ACP use ID=3 for
    //read from ACP and ID=4 for write to ACP
    int ACPidRD;
    int ACPidWR;
    ACPidRD = 3;
    ACPidWR = 4;
...
static ssize_t lockdown_acp_show(struct kobject *kobj,
  struct kobj_attribute *attr, char *buf)
{
   return sprintf(buf, "%d\n", lockdown_acp);
}
